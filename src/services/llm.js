...
class LLMService {
  ...
  async chat(messages, options = {}) {
    ...
    const configured = [];
    const missing = [];
    if (config.llm.openrouter.apiKey) configured.push("OpenRouter"); else missing.push("OpenRouter");
    if (config.llm.openai.apiKey) configured.push("OpenAI"); else missing.push("OpenAI");
    if (config.llm.ollama.enabled) configured.push("Ollama"); else missing.push("Ollama");

    const details = errors.length ? errors.map(e => `  • ${e}`).join("\n") : "  • No provider error was returned.";

    const helpfulMessage = `**🧠 AI Provider Unavailable**

Configured providers: ${configured.length ? configured.join(", ") : "none"}
Missing configuration: ${missing.length ? missing.join(", ") : "none"}

**Actual provider errors:**
${details}

Gobi is running. This is an LLM provider failure, not proof that an API key is missing. No Fly.io deployment is required.`;
